...
public class QuickSort {
	...
	public static void main(String[] args) {
		...
		System.out.print("Display array after sorting: ");
		for (int i = 0; i < ar.length; i++)
			System.out.print(ar[i] + " ");
	}

	/*
	 * method performs quick sort.
	 */
	...
	public static int partitionArray(int pivot, int left, int right) {
		...
		while (true) {
			// find bigger element.
			while (ar[++leftIndicator] < pivot);
				 // execute while loop till -> elements are smaller than pivot as soon as bigger element is found we stop while loop.

			// find smaller element.
			while (rightIndicator > 0 && ar[--rightIndicator] > pivot);
				// execute while loop till -> elements are greater than pivot as soon as smaller element is found we stop while loop
		...
		for (int i = 0; i < ar.length; i++)
			System.out.print(ar[i] + " ");
		System.out.println("Left: " + leftIndicator);
